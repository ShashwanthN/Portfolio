...
const ProjectCard = ({ project, handleProjectClick }) => {
  return (
    ...
    className={`relative border-t border-b p-4 cursor-pointer transition-all duration-150`}
    style={{
  ...
  >
    ...
    <motion.p
      className="mb-1 font-mono text-sm uppercase"
      initial={{ x: -100, opacity: 0 }}
      animate={{ x: 0, opacity: 1 }}
    ...
    <div className="flex flex-wrap mt-2">
      {project.technologies.map((tech) => (
        <span
          key={tech.name}
          className="px-2 py-1 mr-2 mb-2 text-xs"
          style={{
